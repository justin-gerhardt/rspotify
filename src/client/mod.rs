...
use crate::client::http::{HTTPClient, Query};
use crate::json_insert;
use crate::model::*;
use crate::client::oauth2::{Credentials, OAuth, Token};
...
fn get_uri(_type: Type, _id: &str) -> String {
    format!("spotify:{}:{}", _type.to_string(), get_id(_type, _id))
}
...
fn get_id(_type: Type, id: &str) -> String {
...
fn append_device_id(path: &str, device_id: Option<String>) -> String {
...
pub trait BaseClient: HTTPClient {
    ...
    #[maybe_async]
    async fn track(&self, track_id: &str) -> ClientResult<FullTrack> {
        let trid = get_id(Type::Track, track_id);
    ...
    async fn tracks<'a>(
        ...
        for track_id in track_ids {
            ids.push(get_id(Type::Track, track_id));
        }
    ...
    #[maybe_async]
    async fn artist(&self, artist_id: &str) -> ClientResult<FullArtist> {
        let trid = get_id(Type::Artist, artist_id);
    ...
    async fn artists<'a>(
        ...
        for artist_id in artist_ids {
            ids.push(get_id(Type::Artist, artist_id));
        }
    ...
    #[maybe_async]
    async fn artist_top_tracks<T: Into<Option<Country>>>(
        &self,
        artist_id: &str,
        country: T,
    ) -> ClientResult<Vec<FullTrack>> {
    ...
    #[maybe_async]
    async fn artist_related_artists(&self, artist_id: &str) -> ClientResult<Vec<FullArtist>> {
        let trid = get_id(Type::Artist, artist_id);
    ...
    #[maybe_async]
    async fn album(&self, album_id: &str) -> ClientResult<FullAlbum> {
        let trid = get_id(Type::Album, album_id);
    ...
    async fn albums<'a>(
        ...
        for album_id in album_ids {
            ids.push(get_id(Type::Album, album_id));
        }
    ...
    #[maybe_async]
    async fn album_track<L: Into<Option<u32>>, O: Into<Option<u32>>>(
        &self,
        album_id: &str,
        limit: L,
        offset: O,
    ...
    #[maybe_async]
    async fn user(&self, user_id: &str) -> ClientResult<PublicUser> {
    ...
    async fn playlist(
        ...
        let plid = get_id(Type::Playlist, playlist_id);
    ...
    #[maybe_async]
    async fn user_playlists<L: Into<Option<u32>>, O: Into<Option<u32>>>(
        &self,
        user_id: &str,
        limit: L,
        offset: O,
    ...
    #[maybe_async]
    async fn playlist_tracks<L: Into<Option<u32>>, O: Into<Option<u32>>>(
        &self,
        playlist_id: &str,
        fields: Option<&str>,
        limit: L,
    ...
    #[maybe_async]
    async fn user_playlist_create<P: Into<Option<bool>>, D: Into<Option<String>>>(
        &self,
        user_id: &str,
        name: &str,
        public: P,
    ...
    #[maybe_async]
    async fn playlist_reorder_tracks<R: Into<Option<u32>>>(
        &self,
        playlist_id: &str,
        range_start: i32,
        range_length: R,
    ...
    /// [Reference](https://developer.spotify.com/web-api/remove-tracks-playlist/)
    #[maybe_async]
    async fn playlist_remove_specific_occurrences_of_tracks(
        &self,
        playlist_id: &str,
        ...
        for track in tracks {
            ...
            if let Some(_uri) = track.get("uri") {
                let uri = get_uri(Type::Track, &_uri.as_str().unwrap().to_owned());
                map.insert("uri".to_owned(), uri.into());
            }
    ...
    #[maybe_async]
    async fn playlist_follow<P: Into<Option<bool>>>(
        &self,
        playlist_id: &str,
        public: P,
    ) -> ClientResult<()> {
    ...
    #[maybe_async]
    async fn track_features(&self, track: &str) -> ClientResult<AudioFeatures> {
        let track_id = get_id(Type::Track, track);
    ...
    async fn tracks_features<'a>(
        ...
        let ids: Vec<String> = tracks
            .into_iter()
            .map(|track| get_id(Type::Track, track))
            .collect();
    ...
    #[maybe_async]
    async fn track_analysis(&self, track: &str) -> ClientResult<AudioAnalysis> {
        let trid = get_id(Type::Track, track);
    ...
    #[maybe_async]
    async fn get_a_show(&self, id: String, market: Option<Country>) -> ClientResult<FullShow> {
        let mut params = Query::new();
        if let Some(market) = market {
    ...
    #[maybe_async]
    async fn get_shows_episodes<L: Into<Option<u32>>, O: Into<Option<u32>>>(
        &self,
        id: String,
        limit: L,
        offset: O,
...
#[maybe_async]
pub trait UserClient: HTTPClient {
    // TODO: Not `ClientResult`
    fn get_token(&self) -> &Token;
    ...
    #[maybe_async]
    async fn current_user_followed_artists<L: Into<Option<u32>>>(
        &self,
        limit: L,
        after: Option<String>,
    ...
    #[maybe_async]
    async fn current_user_saved_albums<L: Into<Option<u32>>, O: Into<Option<u32>>>(
        &self,
        limit: L,
        offset: O,
    ...
    #[maybe_async]
    async fn transfer_playback<T: Into<Option<bool>>>(
        &self,
        device_id: &str,
        force_play: T,
    ) -> ClientResult<()> {
    ...
    #[maybe_async]
    async fn get_saved_show<L: Into<Option<u32>>, O: Into<Option<u32>>>(
        &self,
        limit: L,
        offset: O,
    ...
    #[maybe_async]
    async fn save_shows<'a>(&self, ids: impl IntoIterator<Item = &'a str>) -> ClientResult<()> {
        let joined_ids = ids.into_iter().collect::<Vec<_>>().join(",");
        let url = format!("me/shows/?ids={}", joined_ids);
    ...
    #[maybe_async]
    async fn shuffle(&self, state: bool, device_id: Option<String>) -> ClientResult<()> {
    ...
    #[maybe_async]
    async fn volume(&self, volume_percent: u8, device_id: Option<String>) -> ClientResult<()> {
    ...
    #[maybe_async]
    async fn repeat(&self, state: RepeatState, device_id: Option<String>) -> ClientResult<()> {
        let url = append_device_id(
    ...
    #[maybe_async]
    async fn previous_track(&self, device_id: Option<String>) -> ClientResult<()> {
    ...
    #[maybe_async]
    async fn pause_playback(&self, device_id: Option<String>) -> ClientResult<()> {
    ...
    #[maybe_async]
    async fn next_track(&self, device_id: Option<String>) -> ClientResult<()> {
    ...
    #[maybe_async]
    async fn new_releases<L: Into<Option<u32>>, O: Into<Option<u32>>>(
        &self,
        country: Option<Country>,
        limit: L,
        offset: O,
    ...
    #[maybe_async]
    async fn featured_playlists<L: Into<Option<u32>>, O: Into<Option<u32>>>(
        &self,
    ...
    #[maybe_async]
    async fn current_user_top_tracks<
        L: Into<Option<u32>>,
        O: Into<Option<u32>>,
    ...
    /// [Reference](https://developer.spotify.com/web-api/get-users-top-artists-and-tracks/)
    #[maybe_async]
    async fn current_user_top_artists<
        L: Into<Option<u32>>,
        O: Into<Option<u32>>,
        T: Into<Option<TimeRange>>,
    >(
    ...
    #[maybe_async]
    async fn current_user_saved_tracks<L: Into<Option<u32>>, O: Into<Option<u32>>>(
        &self,
        limit: L,
        offset: O,
    ...
    #[maybe_async]
    async fn current_user_recently_played<L: Into<Option<u32>>>(
        &self,
        limit: L,
    ...
    #[maybe_async]
    async fn current_user_playlists<L: Into<Option<u32>>, O: Into<Option<u32>>>(
        &self,
        limit: L,
        offset: O,
    ) -> ClientResult<Page<SimplifiedPlaylist>> {
    ...
    #[maybe_async]
    async fn recommendations<L: Into<Option<u32>>>(
        &self,
    ...
    #[maybe_async]
    async fn category_playlists<L: Into<Option<u32>>, O: Into<Option<u32>>>(
        &self,
        category_id: &str,
        country: Option<Country>,
        limit: L,
    ...
    #[maybe_async]
    async fn categories<L: Into<Option<u32>>, O: Into<Option<u32>>>(
        &self,
        locale: Option<String>,
        country: Option<Country>,
        limit: L,
...
impl BaseClient for Spotify {
    ...
    fn get_creds(&self) -> &Credentials {
        self.credentials.unwrap()
    }
}

// TODO: no unwraps
impl UserClient for Spotify {
    fn get_token(&self) -> &Token {
        self.token.unwrap()
    }
